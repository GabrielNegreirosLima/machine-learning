...
def simple_cnn_model(add_dropout=False):
    ...
    fc_a = layers.Dense(512,activation="relu", name="FC-1")(achatar)
...
def run_model(model,it_gen_train,it_gen_validation,param_training,
    ...
    if not load_if_exists or not os.path.isfile(str_file_to_save) :
        ...
        model.compile(optimizer=param_training.optimizer,loss=CategoricalCrossentropy(),metrics=[Accuracy()])
        #use o param_training para os parametros steps_per_epoch e epochs
        history = model.fit_generator(it_gen_train,
                                            steps_per_epoch=param_training.int_batch_size,
        ...
        #salve o modelo
        model.save('cats_vs_dogs_fc')
    else:
        #carrega o modelo
        model = load_model('cats_vs_dogs_fc')
    print("Avaliando validação....")
    loss, acc = model.evaluate_generator(history,
                                            steps=param_training.int_batch_size)
    return acc
