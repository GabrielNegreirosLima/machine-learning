...
class Experimento():
    ...
    def calcula_resultados(self)  -> List[Resultado]:
        ...
        for i,fold in enumerate(self.folds):
            ...
            if(self.ClasseObjetivoOtimizacao is not None):
                ...
                best_method = self.ClasseObjetivoOtimizacao.obtem_metodo(self=self.ClasseObjetivoOtimizacao, trial=study.best_trial)
                self.studies_per_fold.append(study)
            else:
                #caso contrario, o metodo, atributo da classe Experimento (sem modificações) é usado
                best_method = ScikitLearnAprendizadoDeMaquina(self.ml_method)
            ...
            resultado = best_method.eval(df_treino=fold.df_treino, df_data_to_predict=fold.df_data_to_predict, col_classe=fold.col_classe, seed=1)
    ...
    def macro_f1_avg(self) -> float:
        ...
        for i,resultado in enumerate(self.resultados):
            _macros.append(resultado.macro_f1)
        return np.average(_macros)
    @property
