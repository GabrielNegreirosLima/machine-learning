...
class Fold():
    def __init__(self,df_treino :pd.DataFrame,  df_data_to_predict:pd.DataFrame,
        ...
        if num_folds_validacao>0:
            num_repeticoes = num_repeticoes_validacao
            val_k = num_folds_validacao
    ...
    def gerar_k_folds(df_dados,val_k:int,col_classe:str,num_repeticoes:int=1,seed:int=1,
        ...
        #...o parametro val_k
        num_instances_per_partition = int(df_dados.shape[0] / val_k)
        #folds de saida
        arr_folds = []
        ...
        for num_repeticao in range(num_repeticoes):
            ...
            for num_fold in range(val_k):
                ...
                df_treino = pd.concat([df_dados_rand,df_to_predict]).drop_duplicates(keep=False)
                #print(df_treino)
