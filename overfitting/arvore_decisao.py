...
def faz_classificacao(x_treino:pd.DataFrame, y_treino:pd.Series, x_teste:pd.DataFrame, y_teste:pd.Series, min_samples_split:float) -> Tuple[List[float],float]:
    ...
    model_dtree = cria_modelo(x_treino, y_treino, min_samples_split=min_samples_split)

    #realiza a predição (use o método predict do modelo)
    y_predicted = model_dtree.predict(x_teste)

    #calcule a acurácia
    hits = np.sum(y_predicted==y_teste)
    acuracia = hits/len(y_teste)
...
def plot_performance_min_samples(X_treino,y_treino,X_teste,y_teste):
    ...
    for min_samples in np.arange(0.001,0.7,0.01):
        y_predicted, ac_teste =  faz_classificacao(X_treino, y_treino, X_teste, y_teste, min_samples_split=min_samples)
        
        y_predicted, ac_treino = faz_classificacao(X_treino, y_treino, X_treino, y_treino, min_samples_split=min_samples)
